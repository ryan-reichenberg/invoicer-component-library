    ...
    const createHeadersFromProps = () => {
      ...
      for (let tableData of props.columns) {
        let value: Column<{}> = {
          ...
          ...(tableData.cellRender && {
            Cell: (props) => tableData.cellRender!(props),
          }),
        };
        headers.push(value);
      }
        ...
        <thead>
          {headerGroups.map((headerGroup) => (
            <tr
              ...
              className="text-left bg-gray-100 text-gray-700 font-semibold uppercase"
            >
              {headerGroup.headers.map((column) => (
                <th className="py-1 px-5" {...column.getHeaderProps()}>
          ...
          {rows.map((row, i) => {
            ...
            return (
              <tr
                {...row.getRowProps({
                  className:
                    (i % 2 ? 'bg-gray-50' : '') + ' text-sm md:text-base',
                })}
              >
                {row.cells.map((cell) => {
                  return (
