...
//   size = "medium",
  ...
  const mode = primary
    ...
    const disabledClasses = disabled ? 'opacity-50 cursor-not-allowed' + (primary ? '' : ' bg-gray-300') : '';
  return (
    <button
      type="button"
      className={'px-4 py-2 rounded-md text-sm align-bottom inline-flex items-center justify-center cursor-pointer leading-5 transition-colors duration-150 font-medium focus:outline-none '+mode+' '+disabledClasses}
      onClick={onClick}
      disabled={disabled}
    >
      {label}
